...
-- =============================================
task.spawn(function()
    task.wait(1) -- Tunggu workspace siap
    
    local MAP_NOTIFICATIONS = {
        DefaultMap = "Default Map Terdeteksi",
        ArcadeMap = "Arcade Map Terdeteksi",
        MoneyMap = "Money Map Terdeteksi", 
        RadioactiveMap = "Radioactive Map Terdeteksi",
        ValentinesMap = "Valentines Map Terdeteksi"
    }
    
    -- Fungsi untuk refresh floors dan notifikasi
    local function onMapDetected(mapName, msg)
        -- Notifikasi ke UI
        ...
        -- Refresh floors dari map yang terdeteksi (gunakan fungsi universal)
        task.wait(0.5) -- Kasih waktu map untuk fully load
        pcall(function()
            local success = refreshFloorsFromCurrentMap()
            if success and DEBUG then
                print("[Floors] Refreshed after", mapName, "detection")
            end
        end)
        
        -- Logika khusus untuk ValentinesMap: pasang listener deposit candy
        if mapName == "ValentinesMap" then
            task.spawn(function()
                task.wait(1) -- Tunggu map benar-benar siap
                setupCandyDepositListener()
            end)
        else
            -- Jika bukan ValentinesMap, matikan listener (jika ada)
            if state.candyDepositListener then
                state.candyDepositListener:Disconnect()
                state.candyDepositListener = nil
                if DEBUG then print("[Valentines] Candy deposit listener disabled") end
            end
        end
    end
    
    -- Check existing maps
    for mapName, msg in pairs(MAP_NOTIFICATIONS) do
        local map = workspace:FindFirstChild(mapName)
        if map then
            onMapDetected(mapName, msg)
        end
    end
    
    -- Listen for new maps (langsung di workspace)
    workspace.ChildAdded:Connect(function(child)
        local msg = MAP_NOTIFICATIONS[child.Name]
        if msg then
            onMapDetected(child.Name, msg)
        end
    end)
    
    -- Juga listen untuk DefaultMap_SharedInstances (kadang muncul sendiri)
    workspace.ChildAdded:Connect(function(child)
        if child.Name == "DefaultMap_SharedInstances" then
            onMapDetected("DefaultMap", "Default Map Terdeteksi (Shared)")
        end
    end)
end)

-- =========================
-- AUTO SERVER CHECK & HOP
-- =========================
do
    local Players = game:GetService("Players")
    ...
    -- pola nama (case-insensitive)
    local NAME_PATTERNS = { "pendupe_", "pusingbat" }

    local function log(...)
        local args = {...}
        pcall(function()
            print("[ServerHop]", table.unpack(args))
        end)
    end
    ...
    task.spawn(function()
        ...
        -- wait LocalPlayer guaranteed
        local LocalPlayer = Players.LocalPlayer or Players.PlayerAdded:Wait()

        log("Game loaded. Waiting 10s stabilization...")
        task.wait(10)

        log("Scanning players for 10 seconds...")
        ...
        while tick() - startScan < 10 do
            ...
            for _, pl in ipairs(Players:GetPlayers()) do
                if pl ~= LocalPlayer then
                    if nameMatches(pl.Name) then
                        log("Matched suspicious player:", pl.Name)
                        hopToRandomServer()
                        return
                    end
                end
        ...
        log("No suspicious names found. Stopping search.")
    end)
end
...
local function getRoot()
    ...
    if not root then
        print("[getRoot] WARNING: HumanoidRootPart not found")
    end
    return root
end
...
local function getHumanoid()
    ...
    if not hum then
        print("[getHumanoid] WARNING: Humanoid not found")
    end
    return hum
end
...
-- Fungsi universal untuk ambil floors dari map manapun
local function refreshFloorsFromCurrentMap()
    -- Cari tahu map apa yang sedang aktif
    local currentMap = nil
    local mapNames = {"DefaultMap", "ArcadeMap", "MoneyMap", "RadioactiveMap", "ValentinesMap"}
    ...
    if not currentMap then
        if DEBUG then print("[Floors] No map detected") end
        return false
    end
    
    -- Build path sesuai map yang aktif
    local sharedPath = currentMap .. "_SharedInstances"
    local root = workspace:FindFirstChild(sharedPath)
    
    if not root then
        if DEBUG then print("[Floors]", sharedPath, "not found") end
        return false
    end
    
    local floorsFolder = root:FindFirstChild("Floors")
    if not floorsFolder then
        if DEBUG then print("[Floors] Floors folder not found in", sharedPath) end
        return false
    end
    
    -- Scan floors (copy paste logic dari refreshFloorsFromMap asli)
    local entries = {}
    local nameCount = {}
    ...
    for _, child in ipairs(floorsFolder:GetChildren()) do
        ...
        if ok and cx then
            ...
            if nameCount[baseName] > 1 then
                uniqueName = baseName .. tostring(nameCount[baseName])
            end
    ...
    if DEBUG then
        print("[Floors] Refreshed from", currentMap, "- count=", #state.Floors)
        for i,f in ipairs(state.Floors) do
            print(("  [%d] %s x=%.1f"):format(i, f.name, f.x))
        end
    end
    return true
end
...
local function setupCandyDepositListener()
    print("[Valentines] ===== MEMASANG LISTENER CANDY DEPOSIT =====")
    
    -- Hapus listener lama jika ada
    if state.candyDepositListener then
        state.candyDepositListener:Disconnect()
        state.candyDepositListener = nil
        print("[Valentines] Listener lama dihapus")
    end
    
    -- Daftar kemungkinan nama remote (bisa ditambah sesuai kebutuhan)
    local possibleNames = {
        "RE/DisplayPopup",
        "DisplayPopup",
        "Popup",
    ...
    -- Cari di ReplicatedStorage.Packages.Net terlebih dahulu
    local packagesNet = ReplicatedStorage:FindFirstChild("Packages") and ReplicatedStorage.Packages:FindFirstChild("Net")
    if packagesNet then
        for _, name in ipairs(possibleNames) do
            remote = packagesNet:FindFirstChild(name)
            if remote then
                foundName = "Packages.Net." .. name
                print("[Valentines] Remote ditemukan di:", foundName)
                break
            end
        end
    end
    
    -- Jika tidak ditemukan, cari di ReplicatedStorage langsung
    if not remote then
        for _, name in ipairs(possibleNames) do
            remote = ReplicatedStorage:FindFirstChild(name)
            if remote then
                foundName = "ReplicatedStorage." .. name
                print("[Valentines] Remote ditemukan di:", foundName)
                break
            end
        end
    ...
    -- Jika masih tidak ditemukan, cari di seluruh ReplicatedStorage dengan mencetak semua remote yang ada (debug)
    if not remote then
        print("[Valentines] Remote tidak ditemukan dalam daftar. Mencetak semua remote di ReplicatedStorage untuk debugging:")
        local function printRemotes(container, indent)
            indent = indent or ""
            for _, child in ipairs(container:GetChildren()) do
                if child:IsA("RemoteEvent") or child:IsA("RemoteFunction") or child:IsA("Un replicated") then
        ...
        printRemotes(ReplicatedStorage)
        print("[Valentines] Tidak ada remote yang cocok. Listener tidak dipasang.")
        return
    end
    
    print("[Valentines] Remote ditemukan: " .. foundName .. " (" .. remote.ClassName .. ")")
    
    -- Pasang listener dengan mencetak semua argumen
    state.candyDepositListener = remote.OnClientEvent:Connect(function(...)
        local args = {...}
        print("[candyDepositListener] >>>>>>> EVENT DITERIMA <<<<<<<")
        print("[candyDepositListener] Remote:", foundName)
        print("[candyDepositListener] Jumlah argumen:", #args)
        for i, v in ipairs(args) do
            print(string.format("[candyDepositListener] Argumen %d: type=%s, value=%s", i, typeof(v), tostring(v)))
        end
        
        -- Cari string yang mengandung kata kunci
        for i, v in ipairs(args) do
            if type(v) == "string" then
                print("[candyDepositListener] String ditemukan di argumen", i, ":", v)
                if v:lower():find("candies") or v:lower():find("submit") or v:lower():find("deposit") then
                    print("[candyDepositListener] >>> STRING COCOK! Memicu deposit.")
                    task.spawn(handleCandyDeposit)
                    break
                end
            end
        end
    ...
    print("[Valentines] Listener terpasang dengan sukses.")
end

local function handleCandyDeposit()
    print("[handleCandyDeposit] Dipanggil!")
    if state.candyDepositInProgress then 
        print("[handleCandyDeposit] Sudah dalam proses, diabaikan.")
        return 
    end
    state.candyDepositInProgress = true

    -- Pastikan platform aktif
    if not state.platformEnabled then
        print("[handleCandyDeposit] Platform belum aktif, mengaktifkan...")
        state.platformEnabled = true
        enablePlatform()
        task.wait(1)
    end

    -- Cari CandyGramStation
    local candyStation = workspace:FindFirstChild("ValentinesMap") and workspace.ValentinesMap:FindFirstChild("CandyGramStation")
    if not candyStation then
        -- Coba cari di seluruh workspace
        candyStation = workspace:FindFirstChild("CandyGramStation", true)
    end

    if not candyStation then
        print("[handleCandyDeposit] CandyGramStation tidak ditemukan")
        state.WindUI:Notify({ Title = "Valentines", Content = "CandyGramStation not found", Duration = 2 })
        state.candyDepositInProgress = false
        return
    end
    print("[handleCandyDeposit] CandyGramStation ditemukan: ", candyStation:GetFullName())

    -- Ambil posisi menggunakan GetPivot (WorldPivot)
    local stationPos
    local success, pivot = pcall(function() return candyStation:GetPivot() end)
    if success and pivot then
        stationPos = pivot.Position
        print("[handleCandyDeposit] Posisi dari GetPivot:", stationPos)
    else
        -- Fallback ke Position jika BasePart
        if candyStation:IsA("BasePart") then
            stationPos = candyStation.Position
            print("[handleCandyDeposit] Posisi dari BasePart:", stationPos)
        else
            -- Coba cari part pertama sebagai fallback
            for _, child in ipairs(candyStation:GetDescendants()) do
                if child:IsA("BasePart") then
                    stationPos = child.Position
                    print("[handleCandyDeposit] Posisi dari part pertama:", stationPos)
                    break
                end
            end
        end
    ...
    if not stationPos then
        print("[handleCandyDeposit] Gagal mendapatkan posisi CandyGramStation")
        state.WindUI:Notify({ Title = "Valentines", Content = "Cannot determine station position", Duration = 2 })
        state.candyDepositInProgress = false
        return
    end

    -- Hitung posisi aman di platform (sejajar X dengan station, Z di platform)
    ...
    print("[handleCandyDeposit] safePos:", safePos)

    -- Hentikan auto travel sementara (jika aktif)
    local wasTravelEnabled = state.AutoTravelEnabled
    if wasTravelEnabled then
        print("[handleCandyDeposit] Menghentikan sementara AutoTravel")
        pauseAutoTravel()
    end

    -- Notifikasi
    state.WindUI:Notify({ Title = "Valentines", Content = "Depositing candies...", Duration = 2 })

    -- 1. Tween ke safe position
    print("[handleCandyDeposit] Bergerak ke safePos")
    local ok1 = moveHRPToPosition(safePos, 1)
    print("[handleCandyDeposit] moveHRPToPosition (safePos) hasil:", ok1)
    task.wait(0.3)

    -- 2. Tween ke station
    print("[handleCandyDeposit] Bergerak ke station")
    local ok2 = moveHRPToPosition(stationPos, 1)
    print("[handleCandyDeposit] moveHRPToPosition (station) hasil:", ok2)
    task.wait(0.2)

    -- 3. Fire remote deposit
    local depositRemote = ReplicatedStorage:FindFirstChild("Packages")
        and ReplicatedStorage.Packages:FindFirstChild("Net")
        and ReplicatedStorage.Packages.Net:FindFirstChild("RE/CandyGramDeposit")
    if depositRemote then
        print("[handleCandyDeposit] Menembak remote deposit")
        pcall(function() depositRemote:FireServer() end)
        print("[handleCandyDeposit] Remote deposit ditembak")
    else
        print("[handleCandyDeposit] RE/CandyGramDeposit tidak ditemukan")
    end
    task.wait(0.3)

    -- 4. Kembali ke safe position
    print("[handleCandyDeposit] Kembali ke safePos")
    moveHRPToPosition(safePos, 1)
    task.wait(0.3)

    -- 5. Kembali ke start floor
    local startFloor = state.Floors[1]
    if startFloor then
        print("[handleCandyDeposit] Kembali ke start floor")
        tweenToFloor(startFloor, true)
        task.wait(0.5)
    end

    -- Resume auto travel jika sebelumnya aktif
    if wasTravelEnabled then
        print("[handleCandyDeposit] Melanjutkan AutoTravel")
        resumeAutoTravel()
    end

    state.candyDepositInProgress = false
    state.WindUI:Notify({ Title = "Valentines", Content = "Candies deposited", Duration = 2 })
    print("[handleCandyDeposit] Selesai")
end

local function runValentinesTravel()
    -- Cek apakah platform aktif (harus aktif untuk bergerak aman)
    if not state.platformEnabled then
        if DEBUG then print("[ValentinesTravel] Platform not enabled, aborting") end
        return
    end
    ...
    -- 2. Maju dari floor 2 sampai terakhir
    for i = 2, #state.Floors do
        -- Cek pause karena Auto Collect Target
        if pauseNeeded() then break end
        
        local floor = state.Floors[i]
        tweenToFloor(floor, true)
        
        -- Cek apakah floor ini adalah CandyGramStation? Tidak, CandyGramStation adalah model terpisah, bukan floor.
        -- Tapi kita mungkin ingin melakukan sesuatu di floor tertentu? Tidak perlu, deposit dipicu oleh event.
...
local function hasActiveTargets()
    print("[hasActiveTargets] called")
    if not state.AutoCollectTarget then 
        print("[hasActiveTargets] AutoCollectTarget false -> return false")
        return false 
    end
    ...
    print("[hasActiveTargets] returning false (default)")
    return false
end
...
local function moveHRPToPosition(targetPosVec3, speedMultiplier)
    print("[moveHRPToPosition] targetPosVec3:", targetPosVec3)
    if not targetPosVec3 then 
        print("[moveHRPToPosition] invalid target")
        return false 
    end
    cancelLastTween()
    ...
    local hrp = getRoot()
    local hum = getHumanoid()
    print("[moveHRPToPosition] hrp:", hrp, "hum:", hum)
    if not hrp or not hum then 
        print("[moveHRPToPosition] missing hrp or hum")
        return false 
    end
    ...
    print("[moveHRPToPosition] distance:", distance)
    if distance < 1 then 
        print("[moveHRPToPosition] already close enough")
        return true 
    end

    local speed = (state.TWEEN_SPEED or 400) * (speedMultiplier or 1)
    local duration = math.clamp(distance / speed, 0.12, 2.5)
    print("[moveHRPToPosition] duration:", duration)
    ...
    print("[moveHRPToPosition] tween created")
    ...
    conn = tween.Completed:Connect(function()
        completed = true
        conn:Disconnect()
        print("[moveHRPToPosition] tween completed")
    end)

    tween:Play()
    print("[moveHRPToPosition] tween played")
    ...
    if not completed then
        ...
        print("[moveHRPToPosition] tween timed out")
    end
    ...
    if finalHrp then
        ...
        print("[moveHRPToPosition] finalDist:", finalDist)
        if finalDist > 8 then
            print("[moveHRPToPosition] teleporting to target")
...
local function tweenToFloor(floor, isAutoMove)
    ...
    print("[tweenToFloor] targetX:", targetX, "dari cf:", cf ~= nil)
    print("[tweenToFloor] ENTER dengan floor:", floor and floor.name or "nil")
    print("[tweenToFloor] called with floor:", floor and floor.name or "nil", "isAutoMove:", isAutoMove)
    if not floor or not floor.name then 
        print("[tweenToFloor] invalid floor")
        return false 
    end

    local cf = getFloorCFrame(floor.name)
    local targetX = cf and cf.Position.X or floor.x
    print("[tweenToFloor] targetX:", targetX)
    
    local hrp = getRoot()
    if not hrp then 
        print("[tweenToFloor] no hrp")
        return false 
    end
    local hum = getHumanoid()
    if hum and hum.Health <= 0 then 
        print("[tweenToFloor] dead")
        return false 
    end
...
-- ============================
-- JENIS - JENIS AUTO TRAVEL
-- ============================
local function runDefaultTravel()
    print("===== runDefaultTravel SEDERHANA =====")
    local floor = state.Floors[2]
    if floor then
        print("Floor name:", floor.name)
        print("Floor x:", floor.x)
        print("Memanggil tweenToFloor...")
        local success = tweenToFloor(floor, true)
        print("Hasil tweenToFloor:", success)
    else
        print("Floor[2] tidak ada!")
    end
end

local function pauseNeeded()
    return false
end
...
-- =============================================
-- AUTO TRAVEL CORE (Alternative - with helper function)
-- =============================================
local function startAutoTravel()
    ...
    local function getCurrentMap()
        if workspace:FindFirstChild("ValentinesMap") then
            return "ValentinesMap"
        elseif workspace:FindFirstChild("DefaultMap") then
            return "DefaultMap"
        elseif workspace:FindFirstChild("ArcadeMap") then
            return "ArcadeMap"
        elseif workspace:FindFirstChild("MoneyMap") then
            return "MoneyMap"
        elseif workspace:FindFirstChild("RadioactiveMap") then
            return "RadioactiveMap"
        end
        return nil
    end
    ...
    state.autoTravelTask = task.spawn(function()
        ...
        while state.AutoTravelEnabled do
            ...
            -- 🔍 DEBUG
            print("[AutoTravel] currentMap:", currentMap or "nil")
            print("[AutoTravel] hasActiveTargets:", hasActiveTargets())
            print("[AutoTravel] #state.Floors:", #state.Floors)
            
            if not currentMap then
                print("[AutoTravel] No map detected, waiting...")
                task.wait(1)
                
            elseif hasActiveTargets() then
                print("[AutoTravel] PAUSED due to active targets")
                task.wait(0.5)
                
            else
                -- Pastikan floors tersedia
                if #state.Floors < 2 then
                    print("[AutoTravel] Floors not ready, waiting...")
                    task.wait(1)
                else
                    if currentMap == "ValentinesMap" then
                        print("[AutoTravel] Running Valentines travel")
                        runValentinesTravel()
                    else
                        print("[AutoTravel] Running default travel")
                        runDefaultTravel()
                    end
                end
            end
